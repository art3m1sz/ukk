            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div class="flex flex-col ml-3">
                        <div class="font-medium text-white leading-none">Delete Pembayaran ID <?php echo $data['id_pembayaran'] ?> ?</div>
                        <input type="hidden" name="id_pembayaran" value="<?php echo $id ?>" required class="form-control">
                ...
                <a href="pembayaran.php" class="flex-no-shrink bg-blue-400 px-5 ml-4 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-blue-400 text-white rounded-full">Back</a>
            </div>
        </div>
    </form>
</div>
...
if (isset($_POST['hapus'])) {
    ...
    $ubah = mysqli_query($koneksi, 'DELETE FROM pembayaran WHERE id_pembayaran="' . $id_pembayaran . '"');
    if ($ubah) {
        echo '
            <script>

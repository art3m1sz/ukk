            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div class="flex flex-col ml-3">
                        <div class="font-medium text-white leading-none">Delete Tagihan ID <?php echo $data['id_tagihan'] ?> ?</div>
                        <input type="hidden" name="id_tagihan" value="<?php echo $id ?>" required class="form-control">
                ...
                <a href="tagihan.php" class="flex-no-shrink bg-blue-400 px-5 ml-4 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-blue-400 text-white rounded-full">Back</a>
            </div>
        </div>
    </form>
</div>
...
if (isset($_POST['hapus'])) {
  ...
  $ubah = mysqli_query($koneksi, 'DELETE FROM tagihan WHERE id_tagihan="' . $id_tagihan . '"');
  if ($ubah) {
    echo '
        <script>
        alert ("Berhasil Menghapus Data Tagihan");

...
<body class = "body bg-white dark:bg-[#0F172A]">
    ...
    <div class = "content ml-12 transform ease-in-out duration-500 pt-20 px-2 md:px-5 pb-4 min-h-screen">
        ...
        <div class="relative flex items-center mt-4 md:mt-0 mb-4">
            ...
            <form id="search-form" action="tagihan.php" method="GET">
        ...
        <table class="min-w-full text-left text-sm overflow-y-scroll font-medium">
          <thead class="border-b font-medium dark:border-neutral-500 dark:text-black">
            <tr class="bg-black text-white bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 dark:from-cyan-500 dark:to-blue-500 dark:text-black">
              ...
              <th scope="col" class="px-6 py-4 text-center ">ID Penggunaan</th>
              <th scope="col" class="px-6 py-4 text-center ">bulan</th>
              <th scope="col" class="px-6 py-4 text-center ">Tahun</th>
              <th scope="col" class="px-6 py-4 text-center ">Jumlah Meter</th>
          ...
          if (isset($_GET['searchquery'])) {
            ...
            $sql = mysqli_query($koneksi, "SELECT * FROM tagihan WHERE bulan like '%" . $search . "%'");
        } else {
            $sql = mysqli_query($koneksi, "SELECT * FROM tagihan ORDER BY id_tagihan");
        }
        while ($data = mysqli_fetch_Array($sql)) {
            echo "<tr class='hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:text-white dark:hover:bg-gradient-to-r dark:hover:from-cyan-500 dark:hover:to-blue-500'>";
                ...
                echo "<td class='px-6 py-4 text-center'>" . $data["id_penggunaan"] . "</td>";
                echo "<td class='px-6 py-4 text-center'>" . $data["bulan"] . "</td>";
                echo "<td class='px-6 py-4 text-center'>" . $data["tahun"] . "</td>";
                echo "<td class='px-6 py-4 text-center'>" . $data["jumlah_meter"] . "</td>";
                echo "<td class='px-6 py-4 text-center'>" . $data["status"] . "</td>";
                echo "<td class='px-6 py-4 text-center'>
              <a class='' href='edit.php?id=" . $data['id_tagihan'] . "'>Edit</a> |
              <a class='' href='delete.php?id=" . $data['id_tagihan'] . "'>Delete</a>
              </td>";
                echo "</tr>";
        }
        ?>
    ...
    if (input.value.trim() === "") {
        window.location.href = "tagihan.php"; // Change this URL as needed
    } else {
        form.submit();
    }
});
